...
> 附：误跑记录 — `fashionpediazero-shot_synonyms_th015_best` 实际加载 **LoRA v1**（`20260614_2020`），mIoU 0.742，**勿与 true zero-shot 混淆**。

**输出目录**：
- `outputs/fashionpedia_zero_shot_synonyms_th015_best/`（**true zero-shot，已补跑**）
- `outputs/fashionpedia_lora_v2_synonyms_th015_best/`（LoRA v2）
- `outputs/fashionpediazero-shot_synonyms_th015_best/`（误标，实为 LoRA v1）
...
3. **公平对比（已完成）**：同阈值 0.15/0.15 下 LoRA v2 相对 true zero-shot **+1.1pp**（0.746 vs 0.735），box IoU **+3.0pp**；LoRA 主要提升定位/分割质量而非召回。整体增幅偏小，**per-category breakdown** 显示收益集中在 garment parts（§13）。
4. **简历可写结论**：
...
5. **实现**：由临时 notebook 式脚本整理为正式脚本 `fashionpedia_category_breakdown.py`，输出 CSV / MD / JSON。
...
- `outputs/category_breakdown_th015_lora_vs_zs_min10/` — **n≥10，18 类（论文/简历用）**
...
论文/README 见 [FASHIONPEDIA.md](FASHIONPEDIA.md)；汇总图见 `outputs/figures/`。

### 13.8 下一步（已完成项见 §16）

- [x] neckline / pocket / jacket **failure case 可视化**
- [x] `--min-count 10` category breakdown
- [ ] 训练时 **加权 garment parts**（尤其 neckline）
